...
router.put("/:id", verifyToken , async (req,res) =>{
  ...
  if(req.profile.role !== "admin"){
    return res.status(200).json({message: `access denied because you are a ${req.profile.role}`})
  }
  ...
  res.status(200).json({message : `object with id _${req.params.id} is edited`})
})
...
router.delete("/:id",verifyToken , async (req,res)=>{
  ...
  res.status(200).json(`object with id ${req.params.id} has been deleted`)
})
